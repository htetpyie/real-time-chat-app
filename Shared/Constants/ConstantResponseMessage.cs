namespace Shared.Constants;

public class ConstantResponseMessage
{
    public const string UserNameAndPasswordIsRequired = "Username and Password are required.";
    public const string InvalidCredential = "Username or password is invalid.";
    public const string UserNotFound = "User is not found.";
    public const string RoleNotFound = "User role is not found.";
    public const string UserNameExisted = "Username is already existed.";
    public const string RegisterSuccessfully = "Registration is successful. Please login using your account.";
}